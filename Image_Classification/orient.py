...
def nnt_test(weight_0_1,weight_1_2): 
  ...
  for i in range(0,len(orientation_nnt)):
      if orientation_nnt[i] == int(test_image_vectors[i][1]):
       count += 1
  print count
  print (float(count)/len(orientation_nnt))*100
 ...
 #   model_file = "adaboost_model.txt"
    ...
    if(model=="nearest"):
        ...
        for i in testfile:
            ...
            if i[0] == orientation:
                count += 1
        print "Accuracy:", (float(count)/float(len(testfile)))*100
        ...
        with open("nnt_model.txt",'r') as f :
            arrays = json.load(f)
            w0 = np.asarray(arrays[0])
            w1 = np.asarray(arrays[1])
        nnt_test(w0,w1)
        with open("nnt_output.txt", 'w') as output_file :
